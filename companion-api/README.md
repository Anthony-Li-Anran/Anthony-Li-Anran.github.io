...
3. 默认勾选“在本标签页记住 API Key”：密钥保存在 `sessionStorage`，刷新和站内换页后继续使用；关闭标签页或点击“清除会话与凭证”后删除。取消勾选则只在当前页面内存中使用。不会调用 `/api/chat`、兑换邀请码或回退到 Anthony 的密钥。会话文字同样只保存在当前标签页。
...
回复协议优先要求单个 `{"segments":[...]}` JSON 对象，温度设为 0.3。解析器同时兼容代码围栏、多行 JSON、JSON 数组、旧的逐行 JSON 与普通文字；表情或动作字段无效时回退到自然状态，不再让整段回答失败。截断 JSON 中已经完整生成的文字会尽量恢复并保留。
